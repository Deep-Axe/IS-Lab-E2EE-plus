# bob.py - Bob with Double Ratchet Implementation
import socket
import json
import os
...
class Bob:
    ...
    def initialize_session_with_alice(self, alice_public_key_b64):
        """Initialize Double Ratchet session with Alice"""
        try:
            ...
            alice_public_key_bytes = b64decode(alice_public_key_b64.encode())
            alice_public_key = x25519.X25519PublicKey.from_public_bytes(alice_public_key_bytes)
            
            # Initialize as receiver (Bob receives first)
            self.ratchet_state = RatchetInitReceiver(shared_secret, alice_public_key)
            self.initialized = True
            
            print(f" Session initialized with Alice")
            return True
    ...
    def get_alice_public_key(self):
        """Get Alice's public key from server"""
        try:
            s = socket.socket()
            s.connect((HOST, PORT))
            ...
            if response["status"] == "ok":
                ...
                print(f"❌ Error getting Alice's public key: {response.get('error', 'Unknown error')}")
                return None
                
        except Exception as e:
            print(f"❌ Error getting Alice's public key: {e}")
            return None
    
    def ensure_session_initialized(self):
        """Ensure Double Ratchet session is initialized"""
        if not self.initialized:
            alice_public_key = self.get_alice_public_key()
            if alice_public_key:
                return self.initialize_session_with_alice(alice_public_key)
            return False
        return True
    ...
    def send_message(self, message):
        ...
        try:
            ...
            AD = b"bob_to_alice"  # Associated data
    ...
    def fetch_messages(self):
        ...
        try:
            ...
            if response["status"] == "ok":
                ...
                if not messages:
                    print("📪 No new messages from Alice")
                else:
                    ...
                    for msg in messages:
                        try:
                            ...
                            if not self.initialized:
                                ...
                                shared_secret = b"alice_bob_shared_secret_demo_key_32bytes!"[:32]
                                alice_dh_key = x25519.X25519PublicKey.from_public_bytes(header.dh)
                                self.ratchet_state = RatchetInitReceiver(shared_secret, alice_dh_key)
                                self.initialized = True
                            ...
                            print(f"  ❌ Failed to decrypt message from Alice: {e}")
            else:
    ...
    def interactive_mode(self):
        """Interactive chat mode for Bob with Double Ratchet"""
